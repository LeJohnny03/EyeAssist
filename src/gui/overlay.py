...
class Overlay:
    ...
    def draw_tracking_info(self, frame, delta_x, delta_y, mouth_opening,
        ...
        """Zeichnet Debug-Info inklusive Head- und Iris-Delta"""
        ...
        cv2.putText(frame, f"Head  dX: {delta_x:.3f}  dY: {delta_y:.3f}",
                    (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
        cv2.putText(frame, f"Iris  dX: {iris_dx:.4f}  dY: {iris_dy:.4f}",
                    (20, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
        cv2.putText(frame, f"Mouth: {mouth_opening:.3f}",
                    (20, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
